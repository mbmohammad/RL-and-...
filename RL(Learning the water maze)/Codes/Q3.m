...
%%
figure
heatmap(0.5:0.05:1.05,0.2:0.05:1.1, heatMap(1:19,1:12))
title("minimum number of iterations needed for reaching the optimal paths heat map")
xlabel("discount factor")
ylabel("learning rate")
%%
figure
plot(0.2:0.05:1.1, mean(heatMap(1:19,1:12),2),'LineWidth',2,'Color','r')
title(["minimum number of iterations needed for reaching" "the optimal paths heat map(mean over discount factors)"], 'interpreter','latex')
xlabel("learning rate",'interpreter','latex')
ylabel("number of iteration",'interpreter','latex')
%%
figure
plot(0.5:0.05:1.05, mean(heatMap(1:19,1:12),1),'LineWidth',2,'Color','r')
title(["minimum number of iterations needed for reaching" "the optimal paths heat map(mean over learning rates)"], 'interpreter','latex')
